...
<template>
  <div class="movie">
    <img :src="`${movie.posterUrl}`" :alt="movie.nameEn" class="movie-img" />
    <div>
      <div class="movie-name">{{ movie.nameEn }} ({{ movie.year }})</div>
      <span class="movie-overview">{{ movie.description }}</span>
      <div class="movie-buttons" v-if="!isSearch">
        <button
          class="btn movie-buttons-watched"
          @click="movieStore.toggleWatched(movie.filmId)"
        >
        ...
        <button
          ...
          @click="movieStore.deleteMovie(movie.filmId)"
        >
          Delete
        </button>
      </div>
